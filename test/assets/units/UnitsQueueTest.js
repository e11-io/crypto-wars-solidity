...
contract('Units Queue Test', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    await userVillage.setInitialBuildings([cityCenter.id, goldStorage.id, crystalStorage.id]);
    await experimentalToken.approve(userVault.address, 1 * ether);
    await userVillage.create('My new village!','Cool player');

    [starter_gold, starter_crystal] = await userResources.calculateUserResourcesCapacity.call(Alice);
  ...
  it('Add unit to queue fails', async () => {
    ...
    const [userGold,,] = await userResources.getUserResources.call(Alice);
    await userResources.consumeGold(Alice, userGold.toNumber());
    ...
    const [ ,userCrystal, ] = await userResources.getUserResources.call(Alice);
    await userResources.consumeCrystal(Alice, userCrystal.toNumber());
    ...
    const [,, userQuantum] = await userResources.getUserResources.call(Alice);
    await userResources.consumeQuantumDust(Alice, userQuantum.toNumber());
  ...
  it('Add unit to queue after last batch is finished', async () => {
    ...
    // The + 1 its because we are adding the guardians to the queue, theres one more block that is mined.
    assert.equal(guardiansStartBlock.toNumber(), archersStartBlock.toNumber() + blocksToMine + 1);
  })

  it('Get battle stats when two units are in queue and second didn\'t produce yet', async () => {
    await unitsQueue.addUnitsToQueue(archer.id, archersAmount);
    await unitsQueue.addUnitsToQueue(guardian.id, guardiansAmount);
  ...
  it('Get unit quantity when batch is finished', async() => {
    await unitsQueue.addUnitsToQueue(archer.id, archersAmount);

    const archerBatchBlocks = archer.stats[stat.blocks] * archersAmount;
    evmMine(archerBatchBlocks + 1);

    const amountOfArchers = await unitsQueue.getUnitQuantity(Alice, archer.id);

    assert.equal(amountOfArchers.toNumber(), archersAmount);
  });

  it('Get unit quantity while producing batch', async() => {
    await unitsQueue.addUnitsToQueue(archer.id, archersAmount);
    ...
    assert.equal((amountOfArchers.toNumber() / 2), amountOfArchers / 2);
  });

  it('Get unit quantity when theres no units ready', async() => {
    await unitsQueue.addUnitsToQueue(archer.id, archersAmount);

    const amountOfArchers = await unitsQueue.getUnitQuantity(Alice, archer.id);

    assert.equal(amountOfArchers.toNumber(), 0); // for coverage

    const archerBatchBlocks = archer.stats[stat.blocks] * archersAmount;
    evmMine(archerBatchBlocks - 1);

    await unitsQueue.addUnitsToQueue(guardian.id, guardiansAmount);
    const amountOfGuardians = await unitsQueue.getUnitQuantity(Alice, guardian.id);
    assert.equal(amountOfGuardians.toNumber(), 0);
