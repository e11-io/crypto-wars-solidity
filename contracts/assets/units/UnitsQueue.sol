...
contract UnitsQueue is NoOwner, Versioned {
   ...
   */
  event AddUnitsToQueue(address indexed _user,
                        uint _id,
                        uint _quantity,
                        uint _startBlock,
                        uint _endBlock);
  ...
  */
  ...
  // Mapping of user -> unit struct array (keeps track of units in construction queue)
  mapping (address => Unit[]) public userUnitsQueue;
  ...
  function addUnitsToQueue(uint _id, uint _quantity) external {
    ...
    userUnitsQueue[msg.sender].push(Unit(
      uint32(_id),
      uint32(_quantity),
      uint64(startBlock),
      uint64(endBlock)
  ...
  function updateQueue(address _user) public {
    ...
    for (uint j = 0; j < i; j++) {
      Unit storage unit = userUnitsQueue[_user][j];
  ...
  function getStartBlock(address _user) internal view returns (uint) {
    ...
    if (length > 0) {
      Unit storage lastUnit = userUnitsQueue[_user][SafeMath.sub(length, 1)];
  ...
  /*
   * @notice Get User Queue Battle Stats.
   * @dev Function to get the amount of health, defense and attack in the units Queue
   * @param _user The address of the user we want the info from. (address)
   * @return Three uints representing the health, defense and attack of the units in Units Queue.
   */
  ...
  /*
   * @notice Get Queue Unit Quantity
   * @dev Function to get the amount of units in the queue by id
   * @param _user The address of the user. (address)
   * @param _id The id of the unit to calculate the quantity. (uint)
   * @return total quantity of the unit (uint)
   */
  function getUnitQuantity(address _user, uint _id) external view returns (uint totalQuantity) {
    uint length = userUnitsQueue[_user].length;
    uint i = 0;
    uint quantity;

		while (i < length) {
      if (userUnitsQueue[_user][i].id == _id) {
        if (userUnitsQueue[_user][i].endBlock < block.number) {
          totalQuantity = totalQuantity.add(userUnitsQueue[_user][i].quantity);
        } else if (userUnitsQueue[_user][i].endBlock >= block.number && i < length) {
          (, quantity) = calculateUnitQuantity(_user, i);
